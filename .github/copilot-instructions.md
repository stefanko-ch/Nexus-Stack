...
- **Plaintext `http://<service>:<port>` between containers on `app-network`.** This is the architecture, not an oversight: every inter-service call in the stack is plaintext, and no stack terminates TLS internally. The trust boundary is the Docker network, with Cloudflare Tunnel and Access at the edge and zero open ports — `stacks/lakekeeper/docker-compose.yml` states it explicitly for its own API. Don't recommend mutual TLS, a service mesh, or an internal CA for a single service; it would be inconsistent with ~85 others and the threat model does not support it. Anyone positioned to sniff `app-network` already has host or container access, from which the same secrets are readable straight out of the `.env` files under `stacks/*/` — strictly easier than intercepting traffic. Raised as CWE-319 on PR #831 and dismissed on these grounds. What IS worth flagging: a *public* route served over plaintext, or a secret written into an image layer, a log line, or a process's argv.
