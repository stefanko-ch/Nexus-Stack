...
| `STACK_ACCENT` | *(unset — panel accent)* | Accent colour for the whole header — heading, its glow, tagline, logo glow and the active nav link: a CSS hex value (`#ff8800`) or a bare colour keyword (`orange`). Anything else is ignored and the header keeps the panel's own green. Note the logo image itself has the green baked in, so only its glow follows. Has no effect without `STACK_LABEL`. |
