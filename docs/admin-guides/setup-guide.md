...
| `STACK_LABEL` | *(unset — no badge)* | Name shown as a badge under "Control Panel", and prefixed to the browser tab title as `[LABEL]`. Two Nexus-Stack panels are otherwise pixel-identical — same logo, same header, same buttons, one of which is **Teardown** — so with two stacks open in neighbouring tabs only the URL tells them apart. Set it on every stack that is not the one you think of as "the" stack, e.g. `CONDUCTOR-STACK`. Baked in at build time, so it is visible before any button is clickable. Unset leaves the panel exactly as it looks today. |
| `STACK_ACCENT` | *(unset — panel accent)* | Colour of the `STACK_LABEL` badge: a CSS hex value (`#ff8800`) or a bare colour keyword (`orange`). Anything else is ignored and the badge keeps the panel's own green. Has no effect without `STACK_LABEL`. |
