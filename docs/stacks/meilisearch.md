...
2. Add the compose file to `_STANDARD_STOP_COMPOSE_FILES` in the same module so the container is stopped BEFORE the rsync. Meilisearch's index format is LMDB-style (memory-mapped); snapshotting a running LMDB is unsafe (page-table corruption / partial-write visibility) — same atomicity rule that forgejo/dify/metabase already follow. Skipping the stop step would produce snapshots that occasionally fail to restore.
