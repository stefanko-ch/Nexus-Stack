...
- **PostgreSQL** — a shared database for stacks that do not bring their own, and for your own tables. Its consumers today are PostgREST and Trino (via its `postgresql` catalog). Wiki.js, Gitea, Superset, Dagster, OpenMetadata, Soda and Evidence each ship their own PostgreSQL container instead, at their own version
...
| **[QuestDB](https://github.com/stefanko-ch/Nexus-Stack/blob/main/docs/stacks/questdb.md)** | `questdb/questdb:10.0.1` | Column-oriented time-series database with a built-in web console. Speaks the PostgreSQL wire protocol, so psycopg2 and JDBC clients work unchanged, and the InfluxDB line protocol, so Telegraf writes to it without a new plugin. |
...
| **[PostgreSQL](https://github.com/stefanko-ch/Nexus-Stack/blob/main/docs/stacks/postgres.md)** | `postgres:18-alpine` | A shared relational database for stacks that do not bring their own, and for your own tables. Used today by PostgREST and Trino (through its `postgresql` catalog); every other stack listed here ships its own PostgreSQL container at its own version. Internal-only — no public endpoint. |
...
| **[Apache Spark](https://github.com/stefanko-ch/Nexus-Stack/blob/main/docs/stacks/spark.md)** | `nexus-spark:4.2.0-python3.13` | Distributed batch (+ streaming) processor, Master + Worker cluster. Nexus-built image includes PySpark, JupyterLab integration, and S3A connectors for MinIO/Hetzner Object Storage. |
...
| **[OpenSearch](https://github.com/stefanko-ch/Nexus-Stack/blob/main/docs/stacks/opensearch.md)** | `opensearchproject/opensearch:2.19.6` | Apache-2.0 fork of Elasticsearch 7.10, shipped with its Dashboards UI. Speaks the Elasticsearch REST API, so opensearch-py, elasticsearch-py, Logstash and Fluent Bit all work unchanged. Separate from the OpenSearch inside the Marquez stack. |
...
| **[CyberChef](https://github.com/stefanko-ch/Nexus-Stack/blob/main/docs/stacks/cyberchef.md)** | `mpepping/cyberchef:v10.24.0` | GCHQ's "Cyber Swiss Army Knife" — drag-and-drop ~400 operations into a recipe (encode/decode/hash/regex/crypto/compress/parse). The container serves static assets only; operations execute client-side in the browser (a small subset like DNS-over-HTTPS or HTTP request deliberately call out). Perfect for decoding JWTs, base64+gzip payloads, or live-demoing format conversions in class. |
