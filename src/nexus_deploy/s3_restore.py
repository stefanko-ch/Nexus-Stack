...
def render_combined_restore_script(
    *,
    ...
    1. Writes the rclone config to
       ``~/.config/rclone/rclone.conf`` by writing a same-directory
       ``mktemp`` file and renaming it into place. That carries 0600
       from the first byte (mktemp's construction, preserved by
       rename), leaves no unlink-then-reopen window in which a
       symlink could be planted at the destination, and is atomic
       for a concurrent reader. An ``EXIT`` trap removes the temp
       file, so a mid-script failure cannot strand credentials under
       ...
       An earlier revision wrote directly at the target path under
       ``umask 077``, which was adequate for the stated threat model
       (single-user server) but gave up atomicity and left a brief
       window between ``rm -f`` and the reopen.
    ...
    return (
        ...
        "set -euo pipefail\n"
        "\n"
        "# ---- write rclone config (mode 600, atomically) --------\n"
        "# Write to a fresh same-directory temp file and rename it\n"
        "# into place. Three properties, each of which a simpler form\n"
        "# gives up:\n"
        "#\n"
        "#   mode 0600 from the first byte — mktemp creates with that\n"
        "#   mode by construction, and rename preserves the inode, so\n"
        "#   there is never a moment when the credentials sit in a\n"
        "#   world-readable file.\n"
        "#\n"
        "#   no unlink-then-reopen window — mktemp opens O_EXCL under\n"
        "#   a name that did not exist, so there is nothing for a\n"
        ...
        "#\n"
        "# Same shape as the atomic write in nexus_deploy.secret_sync.\n"
        "#\n"
        ...
        'RCLONE_CONF_TMP=$(mktemp "$HOME/.config/rclone/.rclone.conf.XXXXXX")\n'
        "# A failure between here and the rename would otherwise leave\n"
        "# a file full of credentials behind under a new name each run.\n"
...
def render_combined_snapshot_script(
    *,
    ...
    1. Writes the rclone config to ``~/.config/rclone/rclone.conf``
       via same-dir ``mktemp`` + rename — same atomic-write pattern
       as :func:`render_combined_restore_script`, including why it is
