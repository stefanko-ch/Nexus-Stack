...
def _render_lakekeeper(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    LAKEKEEPER__PG_ENCRYPTION_KEY reuses the DB password rather than adding a
    field. Lakekeeper encrypts stored warehouse credentials with it, and warns
    loudly at startup when it is left unset: "THIS IS UNSAFE! Using default
    encryption key for secrets in postgres". Both values are generated by the
    same `random_password` and live in the same Postgres, so a separate secret
    would widen the blast radius of a leak by exactly nothing while adding a
    field that can drift. It must stay stable across spin-ups or previously
    stored credentials become undecryptable -- which it does, because Tofu
    keeps the password in state.

    The R2 block is deliberately NOT passed to the container. Lakekeeper does
    not read storage config from the environment; a warehouse carries its own
    storage profile, created over the management API by
    ``render_lakekeeper_hook``. The bucket and keys therefore reach the hook
    from NexusConfig directly and never enter this stack's .env, which keeps
    the R2 secret out of a file on the server.
    """
    if _empty(c.lakekeeper_db_password):
        raise ServiceEnvError(
            ...
            "random_password.lakekeeper_db_password and the same run pushes "
    ...
    return RenderedEnv(
        env_vars={
            ...
            "LAKEKEEPER_PG_ENCRYPTION_KEY": c.lakekeeper_db_password or "",
        },
    )
...
def _render_nussknacker(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    The image's own ``users.conf`` defines admin/admin, reader/reader,
    writer/writer, deployer/deployer and demo/demo -- five accounts whose
    password equals their username, one of them the literal ``admin``
    that CLAUDE.md forbids. Pointing ``AUTHENTICATION_USERS_FILE`` at a
    rendered file replaces all of them at once: measured against the
    image, ``admin:admin`` then answers 401 and only this account works.

    The file has to be *rendered* rather than committed with a
    ``${VAR}`` placeholder. Nussknacker parses it without environment
    substitution and exits 1 during startup if it finds one -- also
    measured, from the container's own stack trace at
    ``AuthenticationConfiguration.usersOpt``.

    Mode 0o600: the password is in cleartext here, the same reason
    _render_sftpgo restricts its own file. The container bind-mounts it
    read-only.

    No env_vars: everything else this service needs is static and lives
    in the compose file. The .env exists only so the sidecar has
    somewhere to be written alongside.
    """
    if _empty(c.nussknacker_admin_password):
        raise ServiceEnvError(
            "Nussknacker enabled but NUSSKNACKER_ADMIN_PASSWORD is empty -- "
            "it comes from the tofu output `nussknacker_admin_password`. "
            "Starting without it would fall back to the image's demo "
            "accounts, one of which is admin/admin."
        )
    password = c.nussknacker_admin_password or ""
    users_conf = (
        "# Generated by nexus_deploy service_env._render_nussknacker.\n"
        "# Replaces the five demo accounts shipped in the image, whose\n"
        "# passwords equal their usernames. Do not edit by hand: this file\n"
        "# is rewritten on every deploy.\n"
        "users: [\n"
        "  {\n"
    ...
    return RenderedEnv(
        env_vars={},
        sidecars=(SidecarFile(relative_path="users.conf", content=users_conf, mode=0o600),),
        mode=0o600,
    )
...
def _render_questdb(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    if _empty(c.questdb_pg_password):
        raise ServiceEnvError(
            ...
            "`tofu apply` in tofu/stack generates random_password.questdb_pg "
            "and the same run pushes it to Infisical; an empty value here "
            "means one of those did not complete, so check both steps in "
            "this run's log. Aborting rather than starting the "
            "PostgreSQL-wire listener with an empty password.",
        )
    return RenderedEnv(
        env_vars={
...
def _render_opensearch(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    if _empty(c.opensearch_admin_password):
        raise ServiceEnvError(
            ...
            "`tofu apply` in tofu/stack generates "
            "random_password.opensearch_admin and the same run pushes it to "
            "Infisical; an empty value here means one of those did not "
            "complete, so check both steps in this run's log. Aborting: the "
            "node refuses to start without "
...
def _render_marquez(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    Neither is a *Marquez* credential: Marquez ships no user management at
    all, so there is nothing to log into. Each secret authenticates one
    backing service (the OpenSearch one is that node's admin account).

    Cloudflare Access gates the published browser route and nothing else.
    The API sits on app-network with no published port, so any other
    container reaches it without credentials or an Access session — the
    boundary is the Docker network, not Access. Same model and same caveat
    as Lakekeeper.
    ...
    if _empty(c.marquez_db_password):
        raise ServiceEnvError(
            ...
            "`tofu apply` in tofu/stack generates "
            "random_password.marquez_db_password and the same run pushes it "
    ...
    if _empty(c.marquez_opensearch_password):
        raise ServiceEnvError(
            ...
            "`tofu apply` in tofu/stack generates "
            "random_password.marquez_opensearch_admin and the same run "
            "pushes it to Infisical; an empty value here means one of those "
            "did not complete, so check both steps in this run's log. "
            "OpenSearch refuses to start without "
