...
def _render_hedgedoc(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    ...
    if _empty(c.hedgedoc_session_secret) or _empty(c.hedgedoc_db_password):
        ...
        raise ServiceEnvError(
            ...
            "`tofu apply` (initial-setup workflow) to generate "
            "random_password.hedgedoc_session_secret / hedgedoc_db_password "
...
def _render_litellm(c: NexusConfig, e: BootstrapEnv) -> RenderedEnv:
    """LiteLLM Proxy: env vars + a generated ``config.yaml`` sidecar
    that maps OpenAI-compatible model names to backend providers.

    The shipped config.yaml is intentionally minimal — it routes
    ``gpt-3.5-turbo`` → local Ollama (free, in-stack). Operators add
    more providers (real OpenAI, Anthropic, etc.) by editing the
    rendered file or via the /ui at runtime; the in-DB
    STORE_MODEL_IN_DB persists UI-added models across restarts.
    ...
    if _empty(c.litellm_master_key) or _empty(c.litellm_salt_key) or _empty(c.litellm_db_password):
        ...
        raise ServiceEnvError(
            ...
            "random_password.litellm_* resources + push to Infisical, "
            "then re-run spin-up. Aborting to avoid an auth-bypass / "
            "DB-auth-failure container.",
        )
    # Minimal starter config: route OpenAI-compatible model names to
    # the in-stack Ollama. Operators extend via /ui at runtime; the
    # shipped file is just the skeleton so the proxy comes up healthy
    # with at least one model. ``model_name`` is what clients pass in
    # the OpenAI SDK; ``litellm_params.model`` is the LiteLLM-internal
    # provider/model selector (``ollama/<model>`` for the Ollama
    # backend). ``api_base`` is the in-stack Ollama URL.
    config_yaml = (
        "# LiteLLM Proxy — model routing table\n"
        "#\n"
        "# Generated by nexus_deploy on every spin-up. Edits made here\n"
        "# are overwritten next deploy — for persistent operator\n"
        "# overrides, use the /ui at runtime (STORE_MODEL_IN_DB=true\n"
        "# persists DB-added models across restarts).\n"
        "#\n"
        "# Pattern: every entry maps a client-facing OpenAI-compatible\n"
        "# `model_name` to a backend provider via `litellm_params.model`.\n"
        "# To add OpenAI/Anthropic/etc., add entries that read API keys\n"
        "# from env vars (set them as repo secrets, plumb via spin-up.yml).\n"
        "\n"
        "model_list:\n"
